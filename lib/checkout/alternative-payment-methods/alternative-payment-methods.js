...
class AlternativePaymentMethods extends Emitter {
  ...
  async validateOptions () {
    ...
    if (missingField) {
      throw checkoutError('payment-methods-config-missing', { opt: missingField });
    }
    ...
    this.allowedGatewayTypes.forEach(gatewayType => {
      ...
      if (gatewayMissingField) {
        throw checkoutError('payment-methods-config-missing', { opt: `${gatewayType}.${gatewayMissingField}` });
      }
    });

    if (!this.allowedGatewayTypes.length) {
      throw checkoutError('payment-methods-config-missing', { opt: Object.keys(GATEWAYS).join(', ') });
    }
  }

  async getPaymentMethods () {
    return this.checkout.request.get({
  ...
  async selectGatewayStrategy (gatewayType) {
    ...
    if (!gatewayClass) {
      throw checkoutError('payment-methods-not-available');
    }

    return new gatewayClass(this.options, this.checkout);
  }
  ...
  async tokenizePaymentMethod ({ billingAddress }) {
    return this.checkout.request.post({
  ...
  static validateBillingAddress (billingAddress) {
    ...
    if (invalidProperties.length) {
      throw checkoutError('invalid-billing-address-fields', { field: invalidProperties.join(', ') });
    }
  }
}
