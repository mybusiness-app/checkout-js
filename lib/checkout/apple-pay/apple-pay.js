...
const debug = require('debug')('checkout:apple-pay');
...
export class ApplePay extends Emitter {
  ...
  onPaymentAuthorized (event, { errors } = {}) {
    ...
    this.emit('token', event.payment.checkoutToken, event);
  }
  ...
  token (event) {
    ...
    this.checkout.request.post({
      ...
      data,
        ...
        event.payment.checkoutToken = token;
...
function combinedInputs () {
  ...
  const fraud = this.checkout?.fraud?.params ? this.checkout.fraud.params(inputs) : {};

  if (Object.keys(fraud).length > 0) {
    inputs.fraud = fraud;
  }
  return { ...inputs };
}
