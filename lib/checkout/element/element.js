...
const debug = require('debug')('checkout:element');
...
export default class Element extends Emitter {
  static DATA_ATTRIBUTE_ID = 'checkoutElementId';
  static FRAME_NAME_PREFIX = 'checkout-element--';
  static INSTANCE_REF_NAME = '__checkoutElement';
  static OPTIONS = [
    'displayIcon',
    'inputType',
    'style',
  ...
  constructor ({ elements, ...options }) {
    ...
    this.checkout = elements.checkout;
  ...
  get url () {
    ...
    const path = this.checkout.isCanaryOneThousandthSession ? '/field-canary.html' : '/field.html';

    return this.checkout.url(`${path}#config=${config}`);
  }
   ...
   */
  get config () {
    const { bus, id, checkout, type } = this;
    const { deviceId, sessionId } = checkout;
    return {
      ...this._config,
      busGroupId: bus.groupId,
      deviceId,
      elementId: id,
      checkout: checkout.sanitizedConfig,
      sessionId,
      type
    };
  }
   ...
   * excluding (Checkout) Element frames
   *
   * @private
