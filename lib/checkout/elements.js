...
const debug = require('debug')('checkout:elements');

export function factory (options) {
  return new Elements(Object.assign({}, options, { checkout: this }));
}
 ...
 * @param  {Elements} options.checkout
 */
export default class Elements extends Emitter {
  ...
  constructor ({ checkout }) {
    ...
    this.id = `checkout-elements:${uid()}`;
    this.elements = [];
    this.bus = new Bus({ api: checkout.config.api, role: 'elements' });
    this.bus.add(this);
    this.bus.add(checkout);
