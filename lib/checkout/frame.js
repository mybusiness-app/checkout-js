...
const debug = require('debug')('checkout:frame');
...
/**
 * Frame factory. Binds the caller to the `checkout` {FrameOptions} property
 *
 * @public
...
export function factory (options) {
  options = Object.assign({}, options, { checkout: this });
...
export class Frame extends Emitter {
  ...
  constructor ({ checkout, path, payload = {}, ...options }) {
    super();
    this.checkout = checkout;
    this.id = `${this.checkout.id}-${uid()}`;
    this.name = `checkout-frame-${this.id}`;
    this.config = options;
  ...
  prepare (path, payload) {
    const { name, checkout } = this;
    debug('creating request frame');

    payload.version = checkout.version;
    payload.event = name;
    payload.key = checkout.config.publicKey;

    if (checkout.config.hostname) {
      payload.credentialCheckoutHostname = checkout.config.hostname;
    }
    ...
    this.url = this.checkout.url(path);
  ...
  listen () {
    ...
    if ('documentMode' in document) {
      ...
      relay.src = this.checkout.url('/relay');
      relay.name = `checkout-relay-${this.id}`;
