...
const debug = require('debug')('checkout:fraud');
...
export class Fraud extends Emitter {
  constructor (checkout) {
    ...
    this.attachedNodes = [];
    checkout.ready(this.getCollectorProfiles.bind(this));
  ...
  get shouldCollectKountData () {
    return !!this.checkout.config.fraud.kount.dataCollector;
  }

  get udfParams () {
    const udfData = this.checkout.config.fraud.kount.udf || {};
   ...
   */
  params (data) {
    const { fraud: fraudConfig } = this.checkout.config;
    let fraudParams = [];
  ...
  activateKountProfile (params) {
    ...
    const configuredForm = this.checkout.config.fraud.kount.form;
    const sessionIdInputElement = dom.createHiddenInput({
      'data-checkout': 'fraud_session_id',
   ...
   * @throws {CheckoutError} if a form cannot be discovered
   */
  getHostedFieldParentForm () {
    if (this._form) return this._form;
    const fields = this.checkout.config.fields;
