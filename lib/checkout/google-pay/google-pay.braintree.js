...
const debug = Debug('checkout:google-pay:braintree');
...
export class GooglePayBraintree extends GooglePay {
  ...
  createButton ({ paymentOptions, isReadyToPayRequest, paymentDataRequest: checkoutPaymentDataRequest, buttonOptions }) {
    // allow Braintree to set the tokenizationSpecification for its gateway
    checkoutPaymentDataRequest.allowedPaymentMethods.forEach(method => {
      if (method.tokenizationSpecification) delete method.tokenizationSpecification;
    });
    const paymentDataRequest = this.braintree.googlePayment.createPaymentDataRequest(checkoutPaymentDataRequest);
    debug('Creating button', checkoutPaymentDataRequest, paymentDataRequest);
