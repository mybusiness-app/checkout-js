...
const debug = require('debug')('checkout:google-pay');
...
const validateCheckoutMerchantInfo = (checkoutMerchantInfo) => {
  if (checkoutMerchantInfo.paymentMethods.length === 0) {
...
const getGoogleInfoFromMerchantInfo = (checkoutMerchantInfo, options) => {
  const { siteMode, paymentMethods } = checkoutMerchantInfo;
...
const buildPaymentDataRequest = (googlePay, options) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    resolve(data);
  }).then(data => checkout.request.get({ route: '/google_pay/info', data }))
    .then(checkoutMerchantInfo => validateCheckoutMerchantInfo(checkoutMerchantInfo))
    .then(checkoutMerchantInfo => getGoogleInfoFromMerchantInfo(checkoutMerchantInfo, options));
};
...
export class GooglePay extends Emitter {
   ...
   */
  configure (options) {
    if (options.checkout) this.checkout = options.checkout;
  ...
  token (paymentData) {
    ...
    return this.checkout.request.post({ route: '/google_pay/token', data })
      .catch(err => {
      ...
      .then(token => {
        ...
        paymentData.checkoutToken = token;
