...
const debug = require('debug')('checkout:hostedField');
...
export class HostedField extends Emitter {
  ...
  get url () {
    ...
    return `${this.config.checkout.api}/field.html#config=${config}`;
  }

  get classList () {
    const prefix = 'checkout-hosted-field';
    let classes = [prefix];
  ...
  inject () {
    ...
    if (this.iframeTitle) this.iframe.setAttribute('title', this.iframeTitle);
