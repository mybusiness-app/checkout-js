...
const debug = require('debug')('checkout:hostedFields');
 ...
 * @param {Checkout} options.checkout Associated Checkout instance
 * @public
 */
...
export class HostedFields extends Emitter {
  constructor ({ checkout }) {
    ...
    this.configure(checkout.sanitizedConfig);
    this.inject();
    this.on('hostedFields:configure', opts => {
      this.configure(opts.checkoutConfig);
  ...
  integrityCheck (fields) {
    ...
    if (fields) {
      ...
      const currentSelectors = Object.keys(this.config.checkout.fields).map(key => {
        return this.config.checkout.fields[key].selector;
      }).join('');
  ...
  configure (checkoutConfig) {
    this.config = {
      checkout: clone(checkoutConfig || {})
    };
  }
  ...
  fieldConfig (type) {
    const fields = this.config.checkout.fields;
    const field = fields[type];
    const busGroupId = this.checkout.bus.groupId;
