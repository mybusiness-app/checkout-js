...
const debug = require('debug')('checkout:paypal:strategy:braintree');
...
export class BraintreeStrategy extends PayPalStrategy {
  ...
  start () {
    ...
    const { close } = this.paypal.tokenize(tokenOpts, (error, payload) => {
      ...
      // Tokenize with Checkout
      this.checkout.request.post({
        route: '/paypal/token',
        data: { type: 'braintree', payload },
        done: (error, token) => {
          if (error) return this.error('paypal-braintree-tokenize-checkout-error', { cause: error });
