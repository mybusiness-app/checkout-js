...
const debug = require('debug')('checkout:paypal:strategy:complete');
...
export class CompleteStrategy extends PayPalStrategy {
  ...
  start () {
    ...
    const frame = this.frame = this.checkout.Frame({ path: PATHS.START, payload });
  ...
  withSdk () {
    ...
    return this.checkout.request.post({ route: PATHS.ID_TOKENS })
      .then(({ client_id, id_token }) => this.loadScriptPromise(
        `https://www.paypal.com/sdk/js?client-id=${client_id}&disable-funding=${DISABLE_FUNDING.join(',')}`,
        { attrs: { 'data-user-id-token': id_token } })
      )
      .then(() => window.paypal);
  }
}
