...
const debug = require('debug')('checkout:pricing:checkout:attachment');
...
export default class Attachment extends Emitter {
  constructor (pricing, container) {
    ...
    this.checkout = pricing.checkout;
  ...
  get elements () {
    ...
    let elements = { all: [].slice.call(this.container.querySelectorAll('[data-checkout]')) };

    elements.all.forEach(node => {
      const name = dom.data(node, 'checkout');
  ...
  onInputChange (event) {
    ...
    flatten([elems.plan, elems.plan_quantity, elems.addon, elems.tax_code]).forEach(el => {
      if (!el) return;
      if (dom.data(el, 'checkoutSubscription')) return;
      dom.data(el, 'checkoutSubscription', orphanedSubscriptionId);
    });

    // Collect subscription elements, grouped by their ids,
    // then removes elements not paired to a subscription
    const subscriptionGroups = groupBy(elems.all, el => dom.data(el, 'checkoutSubscription'));
    ...
    const adjustmentElemIds = (elems.adjustment || []).map(el => dom.data(el, 'checkoutAdjustment'));
    ...
    Promise.all(Object.keys(subscriptionGroups).map(id => {
      const subscriptionElems = groupBy(subscriptionGroups[id], el => dom.data(el, 'checkout'));

      // Look for an existing subscription for this code
      let subscription = this.pricing.findSubscriptionById(id);

      if (!subscription) {
        subscription = new SubscriptionPricing(this.checkout, { id });
      ...
      return subscription
        ...
        .then(() => {
          ...
          return Promise.all(subscriptionElems.addon.map(addonElem => {
            const code = dom.data(addonElem, 'checkoutAddon');
      ...
      .then(() => {
        ...
        return Promise.all(elems.adjustment.map(adjustmentElem => {
          const id = dom.data(adjustmentElem, 'checkoutAdjustment');
          const itemCode = dom.data(adjustmentElem, 'checkoutAdjustmentItemCode');
          const amount = dom.data(adjustmentElem, 'checkoutAdjustmentAmount');
          const quantity = dom.value(adjustmentElem) || 0;
          const currency = dom.data(adjustmentElem, 'checkoutAdjustmentCurrency');
          const taxCode = dom.data(adjustmentElem, 'checkoutAdjustmentTaxCode');
          const taxExempt = dom.data(adjustmentElem, 'checkoutAdjustmentTaxExempt');
  ...
  updateOutput (price) {
    ...
    if (elems.addon_price) {
      elems.addon_price.forEach(elem => {
        const subscriptionId = dom.data(elem, 'checkoutSubscription');
        const subscription = subscriptionId && this.pricing.findSubscriptionById(subscriptionId);
        if (!subscription) return;
        if (!subscription.isValid) return;
        const addonPrice = subscription.price.base.addons[dom.data(elem, 'checkoutAddon')];
