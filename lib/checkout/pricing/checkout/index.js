...
const debug = require('debug')('checkout:pricing:checkout-pricing');
 ...
 *
 * ```
 * const checkoutPricing = checkout.Pricing.Checkout();
 * checkoutPricing.on('change', price => console.log(price));
 * checkoutPricing
 *   .currency('USD')
 *   .subscription(checkout.Pricing.Subscription().plan('my-plan').done())
...
export default class CheckoutPricing extends Pricing {
  constructor (...args) {
    super(...args);
    this.debug = debug;
    this.checkout.report('pricing:checkout:create');
  }
    ...
    return new PricingPromise((resolve, reject) => {
      if (!(subscription instanceof SubscriptionPricing)) {
        return this.error(errors('invalid-option', {
          name: 'subscription',
          expect: 'a {checkout.Pricing.Subscription}'
        }), reject, 'subscription');
      }
  ...
  adjustment ({ itemCode = null, amount, quantity, id = uid(), currency, taxExempt, taxCode }) {
    ...
    return new PricingPromise((resolve, reject) => {
      ...
      new Promise((resolve, reject) => {
        ...
        return checkout.item({ itemCode }).then(item => {
  ...
  coupon (couponCode) {
    ...
    return new PricingPromise(resolve => resolve(), this)
      ...
      .then(() => new PricingPromise((resolve, reject) => {
        ...
        this.checkout.coupon({ plans: this.subscriptionPlanCodes, coupon: couponCode }, (err, coupon) => {
          if (err) {
  ...
  giftCard (code) {
    ...
    return new PricingPromise((resolve, reject) => {
      ...
      this.checkout.giftCard({ code }, (err, giftCard) => {
        if (err) {
  ...
  bindReporting () {
    ...
    const report = (...args) => this.checkout.report(...args);
