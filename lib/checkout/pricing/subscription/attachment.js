...
const debug = require('debug')('checkout:pricing:attachment');
...
export default class Attachment extends Emitter {
  ...
  change (event) {
    ...
    const targetName = dom.data(target, 'checkout');
    ...
    if (updateAddon) {
      pricing = pricing.then(() => {
        return Promise.all(elems.addon.map(elem => {
          ...
          const code = dom.data(elem, 'checkoutAddon');
  ...
  update (price) {
    ...
    if (elems.addon_price) {
      elems.addon_price.forEach(elem => {
        const addonPrice = price.base.addons[dom.data(elem, 'checkoutAddon')];
