...
const debug = require('debug')('checkout:pricing:subscription-pricing');
...
export default class SubscriptionPricing extends Pricing {
  constructor (checkout, { id = uid() } = {}) {
    super(checkout);
    this.id = id;
    this.debug = debug;
    this.checkout.report('pricing:subscription:create');
  }
   ...
   */
  plan (...args) {
    let { currentPlan, quantity, planCode, done } = this.resolvePlanOptions(...args);
    ...
    return new PricingPromise((resolve, reject) => {
      ...
      this.checkout.plan(planCode, (err, plan) => {
  ...
  */
  giftcard (code, done) {
    ...
    return new PricingPromise(function (resolve, reject) {
      ...
      self.checkout.giftCard({ code }, function (err, gift_card) {
  ...
  coupon (newCoupon, done) {
    ...
    return new PricingPromise((resolve, reject) => {
      ...
      if (typeof newCoupon === 'string') {
        this.checkout.coupon({ plan: this.items.plan.code, coupon: newCoupon }, receiveCoupon);
      } else {
