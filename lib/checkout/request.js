...
import makeJsonpRequest from 'jsonp';
import Promise from 'promise';
import qs from 'qs';

const debug = require('debug')('checkout:Request');
 ...
 * the Checkout /js/v1 API contract
 *
 * @param {Object} options
 * @param {Checkout} options.checkout Checkout instance
 */
export class Request {
  static makeJsonpRequest = makeJsonpRequest;
   ...
   */
  get timeout () {
    return this.checkout.config.timeout || 60000;
  }
   ...
   */
  get key () {
    return this.checkout.config.publicKey;
  }
   ...
   */
  get version () {
    return this.checkout.isParent ? this.checkout.version : this.checkout.config.parentVersion;
  }
   ...
   */
  get shouldUseXHR () {
    return !!this.checkout.config.cors;
  }
   ...
   */
  get isConfigured () {
    return !!this.checkout.configured;
  }
  ...
  request ({ method, route, data = {} }) {
    ...
    const url = this.checkout.url(route);
  ...
  queued (...args) {
    ...
    return new Promise((resolve, reject) => {
      this.checkout.once('configured', () => this.request(...args).then(resolve, reject));
    });
  }
   ...
   */
  xhr ({ method, url, data }) {
    const error = (...args) => this.checkout.error(...args);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.checkout.config.hostname && req.setRequestHeader) {
        req.setRequestHeader('Checkout-Credential-Checkout-Hostname', this.checkout.config.hostname);
      }
  ...
  jsonp ({ method, url, data }) {
    const { timeout, checkout } = this;

    if (method === 'post') data._method = method;
    url += `?${qs.stringify(data, { encodeValuesOnly: true })}`;

    return new Promise((resolve, reject) => {
      Request.makeJsonpRequest(url, { prefix: '__rjs', timeout }, function (err, res) {
        if (err) return reject(err);
        if (res.error) {
          reject(checkout.error('api-error', res.error));
        } else {
          resolve(res);
        }
      });
