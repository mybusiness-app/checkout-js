...
const debug = require('debug')('checkout:risk');

export function factory (options) {
  return new Risk({ checkout: this, ...options });
}
...
export class Risk {
   ...
   * @return {Promise}
   */
  static preflight ({ checkout, number, month, year, cvv, addressFields }) {
    const data = {};

    if (checkout.config.risk.threeDSecure.proactive.enabled) {
      data.proactive = true;
      data.gateway_code = checkout.config.risk.threeDSecure.proactive.gatewayCode;
    }

    return checkout.request.get({ route: '/risk/preflights', data })
      .then(({ preflights }) => {
        debug('received preflight instructions', preflights);
        return ThreeDSecure.preflight({ checkout, number, month, year, cvv, preflights, addressFields });
      })
      .then(({ tokenType, risk }) => ({
