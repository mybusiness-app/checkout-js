...
const debug = require('debug')('checkout:risk:three-d-secure:cybersource');

export default class CybersourceStrategy extends ThreeDSecureStrategy {
  static strategyName = 'cybersource';

  static preflight ({ checkout, number, month, year, gateway_code }) {
    const { preflightDeviceDataCollector } = checkout.config.risk.threeDSecure;

    if (!preflightDeviceDataCollector.enabled) {
    ...
    if ('billingInfoId' in preflightDeviceDataCollector) {
      data.billing_info_id = preflightDeviceDataCollector.billingInfoId;
    }

    return checkout.request.post({ route: '/risk/authentications', data })
      .then(({ jwt, deviceDataCollectionUrl }) => {
        const frame = checkout.Frame({
          ...
          const listener = ({ data }) => {
            try {
              ...
              if (body.MessageType === 'profile.completed') {
                ...
                frame.destroy();
                checkout.bus.off('raw-message', listener);
              }
            } catch (e) {
              debug('Error parsing message. May be foreign message, skipping', e);
            }
          };
          checkout.bus.on('raw-message', listener);
        }), new Promise(resolve => {
          setTimeout(() => {
  ...
  attach (element) {
    ...
    const { checkout } = threeDSecure.risk;
    ...
    this.frame = checkout.Frame({ path: '/three_d_secure/start', type: Frame.TYPES.IFRAME, payload, container })
