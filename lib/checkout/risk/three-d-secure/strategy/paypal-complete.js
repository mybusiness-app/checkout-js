...
const debug = require('debug')('checkout:risk:three-d-secure:paypalcomplete');
...
export default class PayPalCompleteStrategy extends ThreeDSecureStrategy {
  ...
  attach (element) {
    ...
    const { checkout } = this.threeDSecure.risk;
    const { config: { publicKey } } = checkout;
    const defaultEventName = 'paypalcomplete-3ds-challenge';

    const finishURL = checkout.url(`/three_d_secure/finish?key=${publicKey}&event=${defaultEventName}`);
