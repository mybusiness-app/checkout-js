...
const debug = require('debug')('checkout:risk:three-d-secure');
...
export class ThreeDSecure extends RiskConcern {
   ...
   * @return {Promise}
   */
  static preflight ({ checkout, number, month, year, cvv, preflights, addressFields }) {
    return preflights.reduce((preflight, result) => {
      return preflight.then((finishedPreflights) => {
        ...
        return strategy.preflight({ checkout, number, month, year, cvv, addressFields, ...result.params })
  ...
  constructor ({ risk, actionTokenId, challengeWindowSize }) {
    ...
    this.checkout.request.get({ route: `/tokens/${actionTokenId}` })
  ...
  createResultToken (results) {
    ...
    return this.checkout.request.post({ route: '/tokens', data });
  }
