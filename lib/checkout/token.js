...
const debug = require('debug')('checkout:token');
...
const tokenizeOnBus = (checkout, { type }) => checkout.config.parent && !TOKEN_TYPES_CREATED_ON_PARENT.includes(type);
const cardTokenization = ({ type }) => !TOKEN_TYPES_CREATED_ON_PARENT.includes(type);
const billingInfoIdPresent = (checkout) => !!checkout.config.risk.threeDSecure.preflightDeviceDataCollector?.billingInfoId;
 ...
 *                                              billing properties via their 'data-checkout'
 ...
 *                                       properties via their 'data-checkout' attributes, or an Object
 ...
 * checkout service response. The generated token is accessed
 * at `response.token`.
 *
 * When using Elements
 *
 ...
 *                                  properties via their 'data-checkout' attributes
 * @param {Function} done callback
 */
function token (customerData, bus, done) {
  ...
  if (cardTokenization(inputs)) {
    ...
    if ((number && month && year) || (cvv && billingInfoIdPresent(this))) {
      const addressFields = {
        first_name,
        last_name,
        address1,
      ...
      return Risk.preflight({ checkout: this, number, month, year, cvv, addressFields })
        .then(({ risk, tokenType }) => {
          inputs.risk = risk;
