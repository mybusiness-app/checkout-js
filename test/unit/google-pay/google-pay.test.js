...
const getBraintreeStub = () => ({
  ...
  googlePayment: {
    create: sinon.stub().resolves({
      createPaymentDataRequest: sinon.stub().callsFake((paymentRequest) => {
        paymentRequest.allowedPaymentMethods.forEach(method => {
          method.tokenizationSpecification = { type: 'PAYMENT_GATEWAY', parameters: { gateway: 'braintree' } };
        });
        return paymentRequest;
      }),
      parseResponse: sinon.stub().resolves({ nonce: 'GATEWAY_TOKEN' }),
    }),
  },
});
...
describe('Google Pay', function () {
  ...
  googlePayTest(INTEGRATION.DIRECT);
  googlePayTest(INTEGRATION.BRAINTREE);
});

function googlePayTest (integrationType) {
    ...
    if (isBraintreeIntegration) {
      describe('when the libs are not loaded', function () {
        beforeEach(function () {
          ...
          this.sandbox.stub(BraintreeLoader, 'loadModules').rejects(new Error('boom'));
        });
      ...
      it('assigns the braintree configuration', function (done) {
        ...
        nextTick(() => assertDone(done, () => {
          ...
          assert.ok(googlePay.braintree.googlePayment);
        }));
      });
    }
    ...
    context('when the requested merchant Google Pay info returns a valid non-empty list of payment methods', function () {
      it('initiates the pay-with-google with the expected Google Pay Configuration', function (done) {
        ...
        nextTick(() => assertDone(done, () => {
          ...
          } else if (isBraintreeIntegration) {
            assert.deepEqual(window.google.payments.api.PaymentsClient.prototype.isReadyToPay.getCall(0).args[0], {
              apiVersion: 2,
              apiVersionMinor: 0,
              allowedPaymentMethods: [
            ...
            it('request to Checkout to create the token with the billing address from the user Payment Data', function (done) {
              this.clickGooglePayButton(result => {
                result.on('token', () => assertDone(done, () => {
                  ...
                  if (isBraintreeIntegration) {
                    expectedCall.data.type = 'braintree';
                    expectedCall.data.paymentData.paymentMethodData.gatewayToken = {
                      nonce: 'GATEWAY_TOKEN',
              ...
              it('request to Checkout to create the token with the billing address from the <form>', function (done) {
                this.clickGooglePayButton(result => {
                  result.on('token', () => assertDone(done, () => {
                    ...
                    assert.equal(callData.state, '');
                    assert.equal(callData.city, '');
                    assert.equal(callData.postal_code, '123');
                    assert.equal(callData.address1, '');
                    assert.equal(callData.address2, '');
                  }));
                });
              });
            });
